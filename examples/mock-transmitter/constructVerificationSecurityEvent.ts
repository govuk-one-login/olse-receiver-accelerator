...
export function constructVerificationFullSecurityEvent(requestId: string, timeStamp: number, verificationReqeuest: SETVerificationRequest)
    : SET {
    const set: SET = {
...
function addStateToVerificationEvent(set: SET, state: string): void {
    const verificationEvent = set.events['https://schemas.openid.net/secevent/ssf/event-type/verification']
