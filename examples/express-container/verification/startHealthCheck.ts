...
export function startHealthCheck(): boolean {
  ...
  const INTERVALS_MILLISECONDS = config.getNumber(ConfigurationKeys.VERIFICATION_INTERVAL);
  const VERIFICATION_ENDPOINT_URL = config.get(ConfigurationKeys.VERIFICATION_ENDPOINT_URL);
