...
describe('createVerificationJwt', () => {
    beforeEach(() => {
        ...
        (generateJWT as jest.MockedFunction<typeof generateJWT>).mockResolvedValue('mock.jwt.token');
    })
    ...
    it('creates JWT with correct structure and state', async () => {
        ...
        const result = await createVerificationJwt(relyingPartyUrl, streamId, { state })
    ...
    it('handles generateJWT errors', async () => {
        ...
        (generateJWT as jest.MockedFunction<typeof generateJWT>).mockRejectedValue(error);
        ...
        await expect(createVerificationJwt(relyingPartyUrl, streamId)).rejects.toThrow('Failed to create verification JWT');

        expect(consoleSpy).toHaveBeenCalledWith('Error creating verification JWT:', error);
    })
})
